...
var GMAIL_QUERY = 'is:unread newer_than:3d from:(bancochile.cl OR bci.cl OR santander.cl OR bancoestado.cl OR itau.cl)';
  ...
  for (var i = 0; i < threads.length; i++) {
    ...
    for (var j = 0; j < messages.length; j++) {
      ...
      try {
        ...
        // Si la transacción se creó exitosamente, mandar mail de notificación
...
function sendNotificationEmail(parsed) {
  ...
  // Logo SVG inline (el mismo logo-icon de rindo)
  var logoSvg = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32" width="32" height="32">'
    ...
    + '<rect x="-11" y="-2.2" width="22" height="4.5" rx="1" fill="#ffffff" transform="rotate(-32)"/>'
    + '<rect x="-9.2" y="-2.2" width="18.5" height="4.5" rx="1" fill="#ffffff" transform="rotate(38)" opacity="0.55"/>'
    + '</g></svg>';
  ...
  var html = ''
    ...
    + '        <tr>'
    + '          <td style="color:#71717a;padding:4px 0">Categoría</td>'
    + '          <td style="color:#fafafa;text-align:right;padding:4px 0">' + category + '</td>'
    + '        </tr>'
    + '        <tr>'
    + '          <td style="color:#71717a;padding:4px 0">Banco</td>'
    + '          <td style="color:#fafafa;text-align:right;padding:4px 0">' + bank + '</td>'
    + '        </tr>'
    + '        <tr>'
    + '          <td style="color:#71717a;padding:4px 0">Tipo</td>'
    + '          <td style="text-align:right;padding:4px 0"><span style="display:inline-block;background:' + typeColor + '22;color:' + typeColor + ';padding:2px 8px;border-radius:4px;font-size:12px;font-weight:600">' + parsed.type + '</span></td>'
    + '        </tr>'
    + '      </table>'
    + '    </td></tr>'
    + '  </table>'
