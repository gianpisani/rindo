...
export function SessionFormDrawer({
  ...
  useEffect(() => {
    ...
    if (session) {
      ...
      setTargetsOpen(!!hasTargets);

      const hasNotes = session.description || session.coach_notes;
      setNotesOpen(!!hasNotes);

      const hasAdvanced =
        session.workout_subtype || session.training_phase || session.garmin_activity_id;
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="overflow-y-auto flex flex-col">
        ...
        <div className="flex-1 space-y-4 mt-4">
          ...
          {/* ── Objetivos (collapsible) ── */}
          <Collapsible open={targetsOpen} onOpenChange={setTargetsOpen}>
            <CollapsibleTrigger className="flex items-center justify-between w-full border-t border-border/30 pt-4 pb-1 group">
          ...
          <Collapsible open={notesOpen} onOpenChange={setNotesOpen}>
            ...
            <CollapsibleContent className="pt-3 space-y-4">
              ...
              <div className="space-y-2">
                <Label className="text-xs text-muted-foreground">Notas del Coach</Label>
                <Textarea
                  value={coachNotes}
